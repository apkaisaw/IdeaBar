  ...
  return (
    <>
      ...
      <aside className="bg-gradient-to-b from-gray-900/40 to-gray-800/30 w-72 h-screen sticky top-0 p-5 flex flex-col overflow-y-auto no-scrollbar shadow-xl border-r border-purple-500/20 backdrop-blur-xl relative">
        ...
        {/* 固定位置的呼吸光晕 */}
        <div className="absolute top-[15%] right-[20%] w-24 h-24 bg-purple-500/30 blur-[25px] rounded-full pointer-events-none" 
             style={{ animation: 'pulse-glow-1 4s infinite ease-in-out' }}></div>
        
        <div className="absolute top-[50%] left-[15%] w-20 h-20 bg-blue-500/30 blur-[20px] rounded-full pointer-events-none" 
             style={{ animation: 'pulse-glow-2 5s infinite ease-in-out' }}></div>
        
        <div className="absolute bottom-[20%] right-[30%] w-28 h-28 bg-indigo-500/30 blur-[22px] rounded-full pointer-events-none" 
             style={{ animation: 'pulse-glow-3 6s infinite ease-in-out' }}></div>
        ...
        <div className="relative mb-8 z-10">
          <Link href="/" passHref className="flex items-center gap-3 mb-6 px-2 group">
            ...
            <div className="flex flex-col">
              <span className="font-extrabold text-transparent text-xl bg-clip-text bg-gradient-to-r from-purple-300 to-pink-200 leading-tight group-hover:from-purple-200 group-hover:to-pink-100 transition-all duration-300">Idea Bar</span>
                ...
                {item.special ? (
                  ...
                  >
                    ...
                    <div className="relative flex items-center gap-2.5 z-10">
                      <SparklesIcon className="w-5 h-5 text-green-400" style={{ animation: 'subtle-pulse 2s infinite ease-in-out' }} />
                      <span className="text-green-300 font-medium group-hover:text-green-200 transition-colors">Upgrade</span>
                    </div>
                    <span className="ml-auto inline-flex items-center justify-center px-2 py-0.5 text-2xs font-medium rounded-full bg-green-500/20 text-green-300 border border-green-500/20 backdrop-blur-sm relative z-10">PRO</span>
                  </Link>
                ) : (
                  // 常规菜单项
                  <Link
